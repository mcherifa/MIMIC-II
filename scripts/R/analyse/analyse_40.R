#######################################################
# Analyse de sensibilité pour gap de 40 min
#######################################################
...
df_final_wide_mean <- aggregate(cbind(hr, spo2, abpmean, abpsys, abpdias) ~ periode + id, 
                                data=df_numerics_observation_40_min, mean, na.rm = T)
...
df_final_wide_var <- aggregate(cbind(hr, spo2, abpmean, abpsys, abpdias) ~ periode + id, 
                               data=df_numerics_observation_40_min, var, na.rm = T)
...
df_final_wide_lineaire <- aggregate(cbind(hr, spo2, abpmean, abpsys, abpdias) ~ periode + id, 
                                    data = df_numerics_observation_40_min, estimate_tendance, temps_observation = 40) 
...
df_final_wide_arma <- aggregate(cbind(hr, spo2, abpmean, abpsys, abpdias) ~ periode + id, 
                                data = df_numerics_observation_40_min, para_arima,temps_observation = 40) 
...
df_final_wide_haar <- aggregate(cbind(hr, spo2, abpmean, abpsys, abpdias) ~ periode + id, 
                                data = df_numerics_observation_40_min, para_haar) 
...
#######################################################
# Modelisation_superlearner toutes les périodes
# save(df_final, file ="df_final_40.Rdata")
...
#                                folds = 5)
#                                
#save(d, f, h, j, file = "toutes_periodes_40.Rdata")
## load(file = "toutes_periodes_40.Rdata")
#lettres = c("d","f","h","j")
...
save(d, f, h, j, file = "une_periode_40.Rdata")
# load(file = "une_periode_40.Rdata")
lettres = c("d","f","h","j")
