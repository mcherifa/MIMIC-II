...
ci_lower = function(p_hat,n){return(p_hat - 1.96 * sqrt(p_hat * (1-p_hat)/n))}
ci_upper = function(p_hat,n){return(p_hat + 1.96 * sqrt(p_hat * (1-p_hat)/n))}
...
training <- data.frame(target = object$outcome_train,
  ...
  summarise( event_pred = sum(prediction) / length(prediction),
             ci_lower_pred = apply(data.frame(event_pred),1, ci_lower, n=length(object$pred_train)),
             ci_upper_pred = apply(data.frame(event_pred),1, ci_upper, n=length(object$pred_train)),
             event_obs = sum(target) / length(target),
             ci_lower_obs = apply(data.frame(event_obs),1, ci_lower, n=length(object$pred_train)),
             ci_upper_obs = apply(data.frame(event_obs),1, ci_upper, n=length(object$pred_train))
  )
...
val_interne <- data.frame(target = object$outcome_test,
  ...
  group_by(pred_cut) %>%
  summarise( event_pred = sum(prediction) / length(prediction),
             ci_lower_pred = apply(data.frame(event_pred),1, ci_lower, n=length(object$pred_test)),
             ci_upper_pred = apply(data.frame(event_pred),1, ci_upper, n=length(object$pred_test)),
             event_obs = sum(target) / length(target),
             ci_lower_obs = apply(data.frame(event_obs),1, ci_lower, n=length(object$pred_test)),
             ci_upper_obs = apply(data.frame(event_obs),1, ci_upper, n=length(object$pred_test))
  )											 
...
val_externe <- data.frame(target = object$outcome_fab, 
  ...
  group_by(pred_cut) %>%
  summarise( event_pred = sum(prediction) / length(prediction),
             ci_lower_pred = apply(data.frame(event_pred),1, ci_lower, n=length(object$pred_fab)),
             ci_upper_pred = apply(data.frame(event_pred),1, ci_upper, n=length(object$pred_fab)),
             event_obs = sum(target) / length(target),
             ci_lower_obs = apply(data.frame(event_obs),1, ci_lower, n=length(object$pred_fab)),
             ci_upper_obs = apply(data.frame(event_obs),1, ci_upper, n=length(object$pred_fab))
  )	
...
ggplot(data=data_calibration, mapping = aes(x = event_pred, y = event_obs)) +
  geom_point() + 	geom_line() +
  facet_wrap(~ type) +
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), linetype = 2) + 	theme(legend.position="none") +
  theme(legend.title=element_blank(),
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
        axis.line = element_line(colour = "black")) +
  xlab("Mean Probability per Quintile") + 
  ylab("True Positive Frequency per Quintile") +
  geom_rect(aes(fill = type),xmin = -Inf,xmax = Inf,
            ymin = -Inf,ymax = Inf,alpha = 0.02) 
ggsave("calibration_plot_haar_une_periode_non_calibree_m1_separe.tiff", units="in", width=10, height=6, dpi=300, compression = 'lzw')
...
#   xlab("Mean Probability per Bin") + 
#   ylab("True Positive Frequency per Bin")
# ggsave("calibration_plot_haar_une_periode_non_calibree_m1.tiff", units="in", width=10, height=4, dpi=300, compression = 'lzw')
...
# 	ggsave("calibration_plot_haar_une_periode_calibree_m1.tiff", units="in", width=10, height=4, dpi=300, compression = 'lzw')
# 
# ggplot(data_calibration, aes(x = factor(pred_cut), event_pred, fill = type)) 	+
# 	geom_bar(stat = "identity", position = "dodge", aes(y = event_obs, group = type), fill = "darkgrey") +
# 	geom_bar(stat="identity", position = "dodge", alpha = 0.6)
...
ggsave("calibration_plot_haar_une_periode_non_calibree_m2.tiff", units="in", width=10, height=7, dpi=300, compression = 'lzw')
...
# ggsave("calibration_plot_haar_une_periode_calibree_m2.tiff", units="in", width=10, height=4, dpi=300, compression = 'lzw')
# 
# ggplot(data_calibration, aes(x = factor(pred_cut), event_pred, fill = type)) 	+
#   geom_bar(stat = "identity", position = "dodge", aes(y = event_obs, group = type), fill = "darkgrey") +
#   geom_bar(stat="identity", position = "dodge", alpha = 0.6)
