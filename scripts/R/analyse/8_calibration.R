###################
# Calibration de tous les modeles 
###################
rm(list = ls())
...
mutate(type = c(rep("Training",4), 
								rep("Internal Validation", 4), 
								rep("External Validation", 4)))
...
ggplot() + 
	...
	ggsave("calibration_plot_haar_une_periode.tiff", units="in", width=10, height=4, dpi=300, compression = 'lzw')
...
ggplot() + 
	...
	panel.background = element_blank(), axis.line = element_line(colour = "black")) +
	xlab("Mean Probability per Bin") + ylab("True Positive Frequency per Bin")
	ggsave("calibration_plot_haar_une_periode_calibree.tiff", units="in", width=10, height=4, dpi=300, compression = 'lzw')
...
ggsave("calibration_plot_haar_une_periode.tiff", units="in", width=10, height=7, dpi=300, compression = 'lzw')
...
ggsave("calibration_plot_haar_une_periode_calibree.tiff", units="in", width=10, height=7, dpi=300, compression = 'lzw')


###
### Calibration toutes les périodes par patient
###
rm(list = ls())
...
ggplot(data = data_calibree, 
  ...
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0.01))
ggsave("calibration_plot_haar_toutes_periodes_calibree.tiff", units="in", width=10, height=7, dpi=300, compression = 'lzw')
