...
# Test 
#cov1tr <- to.balanced(dfessai,match("identif",names(dfessai)),
#match("time_and_date",names(dfessai)),match("hr",names(dfessai)))
#cov2tr <- to.balanced(dfessai,match("identif",names(dfessai)),
#match("time_and_date",names(dfessai)),match("spo2",names(dfessai)))

to.balanced <- function (data, id.col, time.col, Y.col, other.col = NA)
{
  
    if (length(id.col) > 1) {
        stop("Un seul vecteur d'identification du sujet est possible")
    }
    ...
    if (length(time.col) > 1) {
        stop("Un seul vecteur d'identification de temps est possible")
    }
